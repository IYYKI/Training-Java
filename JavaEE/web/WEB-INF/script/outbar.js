...
function cs(id, css) {
    var obj = new DHTMLObject(id);
    obj.init();
    obj.el.className = css;
}

function ci(id, src, over) {
    var obj = new DHTMLObject(id);
    obj.init();
    var npic = new Image();
    var prestr = src.substring(0, src.indexOf(".gif"));
    npic.src = prestr + (over ? '_a' : '') + '.gif';
    obj.el.src = npic.src;
}

function ci_ns(idd, imsrc, over) {
    pos = imsrc.indexOf(".gif");
    prestr = imsrc.substring(0, pos);
    this.npic = new Image();
    this.npic.src = prestr + (over ? '_a' : '') + ".gif";
    idd.src = this.npic.src;
}

function change_stpan_md(idd) {
    cs(idd, 'panel_click')
}

function change_stpan_mout(idd) {
    cs(idd, 'panel')
}

function change_style_md(idd) {
    cs(idd, 'imgstyle_over')
}

function change_style_mout(idd) {
    cs(idd, 'imgstyle')
}

function change_style_opmd(idd, idsrc) {
    ci(idd, idsrc, 1)
}

function change_style_opmout(idd, idsrc) {
    ci(idd, idsrc, 0)
}

function change_style_ns(idd, imsrc) {
    ci_ns(idd, imgsrc, 1)
}

function change_style_ns1(idd, imsrc) {
    ci_ns(idd, imgsrc, 0)
}

function chang_arrd(idd) {
    cs(idd, 'arrowclick')
}

function chang_arru(idd) {
    cs(idd, 'arrow')
}
